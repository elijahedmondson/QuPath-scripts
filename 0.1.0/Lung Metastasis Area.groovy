...
runClassifier('C:\\Users\\edmondsonef\\Desktop\\QuPath\\Wakefield\\PHL 190540\\classifiers\\Lung.Metastasis.qpclassifier');
...
for (annotation in getAnnotationObjects()){
    ...
    annotation.getMeasurementList().putMeasurement("Metastasis Area", totalArea)
...
for (annotation in getAnnotationObjects()){
    //Block 1
    def positiveCells = hierarchy.getDescendantObjects(annotation,null, PathCellObject).findAll{it.getPathClass() == getPathClass("Stroma")}
    double totalArea = 0
    for (def cell in positiveCells){
        totalArea += cell.getMeasurementList().getMeasurementValue("Cell: Area")
    }
    ...
    //Add the total as "Positive Area" to each annotation.
    annotation.getMeasurementList().putMeasurement("Lung Area", totalArea)
    def annotationArea = annotation.getROI().getArea()
